...
int main (int argc, char *argv[]) {
    ...
    cols = BLOCK_SIZE(grid_coords[1],grid_size[1],n);  // 讀取 checkboard的資料 
    ...
    print_checkerboard_matrix((void *) a, mpitype, m, n, grid_comm); //印出 
    
    if (!id) {
        printf("read vector:\n");
    }
    print_block_vector((void *) b, mpitype, nprime, col_comm);
